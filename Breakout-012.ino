...
float ballSpeedMax = 4.0;  // Reduced max speed
...
int paddleSpeedMultiplier = 2;  // Increase paddle speed as ball speed increases
...
// Brick properties - changed for cubes
#define BRICK_ROWS 6
#define BRICK_COLUMNS 10
#define BRICK_WIDTH 57
...
// Expanded Neon Colors for Bricks - 30 Colors
uint16_t neonColors[30] = {
    ...
    tft.color565(255, 140, 0),    // Neon orange
    tft.color565(255, 165, 0),    // Neon bright orange
...
void displayWelcomeScreen() {
  ...
  tft.setCursor(10, 20);
  tft.print("Welcome to Breakout-011!");
...
// Core 1 task for updating display
void core1Task() {
  while (true) {
    if (graphicsReady) {
      updateDisplay();
...
// Core 2 task for handling game logic
void core2Task() {
  while (true) {
    if (graphicsReady) {
      moveBall();
...
// Core M4 task for updating the scoreboard
void coreM4Task() {
  while (true) {
...
void updatePaddleAI() {
  ...
  // Calculate the target X position for the paddle, based on the ball's position
  int targetX = ballX + ballDX * playerIntelligence;

  // Ensure the target position is within the screen bounds
  targetX = constrain(targetX, SCOREBOARD_WIDTH, SCREEN_WIDTH - paddleWidth);

  // Determine the direction the paddle needs to move
  if (paddleX + paddleWidth / 2 < targetX - playerAccuracy) {
    paddleX += min(paddleSpeed, abs(targetX - paddleX - paddleWidth / 2));  // Move right
  } else if (paddleX + paddleWidth / 2 > targetX + playerAccuracy) {
    paddleX -= min(paddleSpeed, abs(targetX - paddleX - paddleWidth / 2));  // Move left
  }

  // Prevent impossible moves - Ensure the paddle stays within the screen bounds
  paddleX = constrain(paddleX, SCOREBOARD_WIDTH, SCREEN_WIDTH - paddleWidth);
