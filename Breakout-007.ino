...
volatile int maxLevelReached = 1;   // New variable to track the highest level reached
...
void updateScoreboard() {
  ...
  tft.setTextColor(tft.color565(255, 215, 0));  // Gold for max level reached
  tft.setTextSize(3);  // Text size for max level reached
  tft.fillRect(10, 360, SCOREBOARD_WIDTH - 20, 50, scoreboardBackgroundColor);  // Clear previous max level reached
...
void nextLevel() {
  ...
  // Update the highest level reached if the current level surpasses it
